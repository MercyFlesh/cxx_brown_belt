...
public:
    ...
    struct WriteAccess {
        ...
        WriteAccess(const K& key, Bucket& bucket)
            : guard(bucket.mx), ref_to_value(bucket.data[key])
        {
        }
    };
    ...
    struct ReadAccess {
        ...
        ReadAccess(const K& key, const Bucket& bucket)
            : guard(bucket.mx), ref_to_value(bucket.data.at(key))
        {
        }
    };
    ...
    {
        return { key, buckets[get_hash_index(key)] };
    }
    ...
    {
        auto& bucket = buckets[get_hash_index(key)];
        lock_guard guard(bucket.mx);
    ...
    {
        MapType resault;
        for (auto& [data, mx] : buckets)
        {
            lock_guard guard(mx);
            resault.insert(data.begin(), data.end());
        }
