...
{
    Domain domain("ya.ru");
    Range reverse_parts_domain = domain.GetReversedParts();
    vector<string> reverse_domain_parts(
        begin(reverse_parts_domain), end(reverse_parts_domain));

    ASSERT_EQUAL(reverse_domain_parts, vector<string>({ "ru", "ya" }));
}
...
void TestDomainCheckerAbsorption()
{
    vector<Domain> domains_check({ Domain("ya.ru"), Domain("m.ya.ru") });
    DomainChecker checker(begin(domains_check), end(domains_check));

    ASSERT(checker.IsSubdomain(Domain("market.ya.ru")));
}

void TestCheckBannedDomains()
{
    vector<Domain> domains_check({ Domain("ya.ru"), Domain("m.ya.ru") });
    ASSERT(!CheckDomains(domains_check, vector({ Domain("shop.ya.ru") })).front());
}

void TestOutput()
{
    stringstream result;
    PrintCheckResults({ true, false }, result);
    ASSERT_EQUAL(result.str(), "Good\nBad\n");
}

void TestReadFirstEmptyDomain()
{
    stringstream input("2\nya.ru\nmicrosoft.com\n");
    ASSERT_EQUAL(ReadDomains(input).front(), Domain("ya.ru"));
}

int main() {
    TestRunner tr;
    RUN_TEST(tr, TestSplitEmpty);
    RUN_TEST(tr, TestGetReversedParts);
    RUN_TEST(tr, TestIsSubDomain);
    RUN_TEST(tr, TestIsSubdomainMixedArgs);
    RUN_TEST(tr, TestDomainCheckerAbsorption);
    RUN_TEST(tr, TestCheckBannedDomains);
    RUN_TEST(tr, TestOutput);
    RUN_TEST(tr, TestReadFirstEmptyDomain);
